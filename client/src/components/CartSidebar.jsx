    ...
    <>
      ...
      >
        ...
        <div className="p-6 border-b border-[#4a3b2a]/10 flex justify-between items-center bg-white">
          <h2 className="text-2xl font-playfair font-bold text-[#4a3b2a]">Your Cart ({cartItems.length})</h2>
          <button onClick={() => setIsCartOpen(false)} className="text-2xl hover:text-[#d4a017] transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {cartItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
               ...
               <button onClick={() => setIsCartOpen(false)} className="mt-4 text-[#d4a017] font-bold underline">Start Shopping</button>
            </div>
          ) : (
            cartItems.map((item) => (
              <div key={item._id} className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-between">
                   ...
                   <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-bold text-[#4a3b2a] line-clamp-1 leading-tight">{item.name}</h4>
                        <p className="text-xs text-stone-400 mt-1 uppercase tracking-wide">{item.category} • {item.weight}</p>
                      </div>
                   ...
                   <div className="flex justify-between items-end mt-3">
                      ...
                      <span className="font-bold text-[#4a3b2a] text-lg">₹{item.price * item.qty}</span>
                   </div>
                </div>
              </div>
            ))
